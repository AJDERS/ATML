...
def optimal_kl(bias, bound, epsilon):
    '''
    ...
    while (step > epsilon):
        ...
        tmp = (1 - bias) / (1 - result)
        below_bound = (
            (bias * math.log(p0 / result) + (1.0 - bias) * math.log(tmp)) < bound
        )
...
def kl_bound(phats, delta, n, lower, epsilon):
    ...
    if not lower:
        bounds = np.array([optimal_kl(phat, bound_for_kl, epsilon) for phat in phats])
    else:
        bounds = np.array([optimal_kl(phat, bound_for_kl, epsilon) for phat in phats]) # DO OTHER CALCULATION HERE
    return zip(phats, bounds)
...
def remove_too_large(zipped_data):
    return [(x, y) for (x, y) in zipped_data if y < 1.0]
...
def make_plot(n = 1000, delta = 0.01, interval=[0.0, 1.0], lower=False, epsilon=2**(-52)):
    ...
    hoef_phats, hoef_bounds = zip(*remove_too_large(hoeffding_bound(phats, delta, n, lower)))
    kl_phats, kl_bounds = zip(*remove_too_large(kl_bound(phats, delta, n, lower, epsilon)))
    pin_phats, pin_bound = zip(*remove_too_large(pinsker_bound(phats, delta, n)))
    ref_pin_phats, ref_pin_bound = zip(*remove_too_large(refined_pinsker_bound(phats, delta, n)))
